...
def calculate_for_app(Dy_t,h_t, shell_diameter, order_metrs):
    out = calculate(
        Dy_t,
        h_t,
        Dy_o,
...
if __name__ == "__main__":
    ...
    print("Результаты тестового расчета:")
    print("="*50)
    
    result_names = [
        "Вес стальной трубы 1мп (кг)",
        "Длина стальной трубы (м)", 
        "Длина ПЭ-оболочки (мм)",
        "Пенокомпоненты (кг)",
        "Компонент А (кг)",
        "Компонент В (кг)", 
        "Количество стальной трубы (кг)",
        "Количество ПЭ оболочки (мп)",
        "Провод медный (кг)",
        "Расход центраторов (м)",
        "ЗП центровщики брутто (тенге/мес)",
        "ЗП заливщик брутто (тенге/мес)",
        "Стоимость работы на 1 трубу (тенге)",
        "Налоги с ФОТ (тенге)",
        "Накладные расходы (тенге)",
        "Себестоимость 1 шт без НДС (тенге)",
        "Себестоимость 1 мп без НДС (тенге)",
        "Себестоимость 1 мп с НДС (тенге)",
        "Стоимость заказа с НДС (тенге)"
    ]
    
    #for i, (name, value) in enumerate(zip(result_names, out)):
    #    print(f"{i+1:2d}. {name}: {value:.4f}")
    
    print("="*50)
    print(f"Итоговая стоимость заказа с НДС: {out[-1]:.2f} тенге")